...
{
    public delegate bool TryParseDelegate<T>(ReadOnlySpan<char> str, [System.Diagnostics.CodeAnalysis.MaybeNullWhen(false)]out T result);

    private static readonly ConcurrentDictionary<Type, UrlValueConstraint> _cachedInstances = new();

    public static bool TryGetByTargetType(Type targetType, [System.Diagnostics.CodeAnalysis.MaybeNullWhen(false)]out UrlValueConstraint result)
    {
    ...
    public abstract bool TryParse(ReadOnlySpan<char> value, [System.Diagnostics.CodeAnalysis.MaybeNullWhen(false)]out object result);
    ...
    private class TypedUrlValueConstraint<T> : UrlValueConstraint
    {
        ...
        public override bool TryParse(ReadOnlySpan<char> value, [System.Diagnostics.CodeAnalysis.MaybeNullWhen(false)]out object result)
        {
    ...
    private sealed class NullableTypedUrlValueConstraint<T> : TypedUrlValueConstraint<T?> where T : struct
    {
        ...
        private static TryParseDelegate<T?> SupportNullable(TryParseDelegate<T> parser)
        {
            ...
            bool TryParseNullable(ReadOnlySpan<char> value, [System.Diagnostics.CodeAnalysis.MaybeNullWhen(false)]out T? result)
            {
                if (value.IsEmpty)
                {
                    result = default;
