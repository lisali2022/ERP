...
public partial class TablesTree
{
    ...
    {
        base.OnInitialized();        
        TreeItems = TreeFoo.GenerateFoos(LocalizerFoo, 3);//数据从数据库中获得        
        TreeItems.AddRange(TreeFoo.GenerateFoos(LocalizerFoo, 2, 1, 100));// 插入 Id 为 1 的子项       
        TreeItems.AddRange(TreeFoo.GenerateFoos(LocalizerFoo, 3, 101, 1010)); // 插入 Id 为 101 的子项
    }
    ...
    /// https://gitee.com/LongbowEnterprise/BootstrapBlazor/blob/main/src/BootstrapBlazor.OnlyServer/Data/Foo.cs
    /// </summary>
    private class TreeFoo : Foo
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<TreeFoo> GenerateFoos(IStringLocalizer<Foo> localizer,
                                                 int count = 80,
                                                 int parentId = 0,
                                                 int id = 0) => Enumerable.Range(1, count).Select(i => new TreeFoo()
        {
            Id = id + i,
            ParentId = parentId,
