...
using BootstrapBlazor.OnlyServer.Extensions;
...
namespace BootstrapBlazor.OnlyServer.Components.Components;
    ...
    private async Task SetCulture(SelectedItem item)
    {
        ...
        else
        {
            ...
            if (SelectedCulture != item.Value)
            {
                ...
                var uriQuery = "/Culture/SetCulture" + query;
                // use a path that matches your culture redirect controller from the previous steps
                NavigationManager.NavigateTo(uriQuery, forceLoad: true);
            }
        }
    }
